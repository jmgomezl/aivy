...
export default function QuorumCanvas(){
 ...
 const [busy,setBusy]=useState(''),[error,setError]=useState(''),[notice,setNotice]=useState(''),[available,setAvailable]=useState(false),[accepted,setAccepted]=useState(false)
 ...
 async function refresh(){
  ...
  try{const data=await api<View>('/');if(alive.current)setView(data)}catch(e){if((e as {status?:number}).status!==401&&alive.current)setError(e instanceof Error?e.message:'Status unavailable.')}
 }
 useEffect(()=>{
 ...
 async function activate(){
  ...
  try{await api('/start',{});setBusy('Saving your monthly rules…');const next=await api<View>('/activate',{...rules,acceptTerms:true});setView(next);setNotice('Rules saved. Every purchase must pass the same limits.')}
 ...
 async function action(name:'pause'|'resume'|'run'){
  ...
  try{const next=await api<View>(`/${name}`,{});setView(next);setNotice(name==='pause'?'New purchases paused. Existing cover and any already accepted purchase remain.':name==='run'&&next.mandate?.status!=='running'?'No extra policy purchased. The agent waits until the next eligible renewal.':'Saved rules are active.')}
 ...
 return <div className="quorum-canvas">
  ...
  <main className="qc-main">
   ...
   <div className="qc-feedback" aria-live="polite">{error&&<p className="qc-warning" role="alert">{error} <button className="qc-text-button" onClick={()=>void refresh()}>Check saved status</button></p>}{notice&&!running&&<p>{notice}</p>}</div>
